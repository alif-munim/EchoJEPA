...
IMG_SIZE = 336 # From color run config
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default="./inference_output_color", help="Dir to save chunked results")
    parser.add_argument('--checkpoint', type=str, default="/mnt/custom-file-systems/efs/fs-0049217cdf69186d7_fsap-0fa7145b64eaa046b/vjepa2/classifier/output/color_run1_convnext_small_336px/epoch_2.pt")
    ...
    # We only use the logger on Rank 0 to keep tqdm clean
    logger = setup_logger(rank) if rank == 0 else None
    
    if rank == 0:
        os.makedirs(args.output_dir, exist_ok=True)
        logger.info(f"Starting COLOR inference on {world_size} GPUs.")
    ...
    results = []
    output_file = os.path.join(args.output_dir, f"color_predictions_rank{rank}.csv")
    buffer_size = 1000
    ...
    # TQDM SETUP
    # -----------------------------------------------------------
    # Only wrap loader with tqdm on Rank 0
    if rank == 0:
        iterator = tqdm(loader, desc="Inference", unit="batch", dynamic_ncols=True)
    else:
        iterator = loader
    ...
    with torch.no_grad():
        for i, (inputs, uris, valids) in enumerate(iterator):
            ...
            # Update TQDM Stats on Rank 0
            if rank == 0 and i > 0 and i % 10 == 0:
                elapsed = time.time() - start_time
                # Calculate Global Rate (approx)
                # Note: i is local step, but multiplied by world size assumes sync. 
                # Distributed inference can desync slightly, but this is a good approximation.
                rate = (i * args.batch_size * world_size) / elapsed
                iterator.set_postfix_str(f"Global Rate: {rate:.1f} vid/s")
    ...
    if rank == 0:
        logger.info("Rank 0 complete. Waiting for others...")
    
    dist.barrier()
    
    if rank == 0:
        logger.info(f"All ranks finished. Results saved to {args.output_dir}/color_predictions_rank*.csv")
