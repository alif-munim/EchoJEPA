"""Build A4C-only train/val/test splits for TAPSE regression on MIMIC echos.

Seeded from `tapse` (all views, 1d match window, physiological range [0.5, 3.5] cm
applied by build_probe_csvs.py). This script narrows further to A4C-only clips
using the ConvNeXt view manifest (view_status == OK).

UHN-trained TAPSE probes were trained on A4C-only B-mode, matching the
cross-institutional transfer protocol used for LVEF/MR/TR.

Labels are carried over verbatim from the all-view tapse split. z-score params
are re-derived from the A4C-filtered train split so stats reflect the actual
evaluation distribution (not used at inference — we supply UHN zscore at test
time so the probe sees inputs in the space it was trained on).

Output: tapse_a4c/{train,val,test}.csv + zscore_params.json + task_meta.json
"""
...
SRC_SPLIT_DIR = REPO_ROOT / "experiments/nature_medicine/mimic/probe_csvs/tapse"
OUT_SPLIT_DIR = REPO_ROOT / "experiments/nature_medicine/mimic/probe_csvs/tapse_a4c"
...
def main() -> None:
    ...
    for split in SPLITS:
        ...
        print(f"  TAPSE:    mean={s['mean']} std={s['std']} median={s['median']}  range=[{s['min']}, {s['max']}]")
