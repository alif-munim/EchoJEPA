"""Build A4C-only (all modes) train/val/test splits for MV E/e' medial on MIMIC.

Seeded from `mv_ee_medial` (all views, 1d match window, physiological range
[1.0, 40.0] ratio applied by build_probe_csvs.py). This script narrows to
A4C-only clips using the ConvNeXt view manifest (view_status == OK).

UHN-trained mv_ee_medial probe was trained on A4C-only with bmode_only=false
(tissue Doppler overlays are permitted, unlike TAPSE). We match that: A4C
only, no color/mode filter.

Labels carried over verbatim from the all-view mv_ee_medial split. z-score
params re-derived from the A4C-filtered train split so stats reflect the
actual evaluation distribution; at inference we supply UHN zscore
(target_mean=11.4293, target_std=5.2505) so the probe sees inputs in the
space it was trained on.

Output: mv_ee_medial_a4c/{train,val,test}.csv + zscore_params.json + task_meta.json
"""
...
SRC_SPLIT_DIR = REPO_ROOT / "experiments/nature_medicine/mimic/probe_csvs/mv_ee_medial"
OUT_SPLIT_DIR = REPO_ROOT / "experiments/nature_medicine/mimic/probe_csvs/mv_ee_medial_a4c"
...
def filter_split(src: Path, a4c: set[str]) -> tuple[list[tuple[str, float]], dict]:
    ...
    stats = {
        ...
        "mv_ee_medial_stats": _stats(vals),
...
def main() -> None:
    ...
    task_meta = {
        ...
        "unit": "ratio",
        "source_measurement": "e_e_prime",
        "note": (
            "MIMIC e_e_prime is the structured-report E/e' ratio. UHN mv_ee_medial "
            "is medial (septal) E/e'. MIMIC entry is ambiguous (medial, lateral, or "
            "average) but distribution matches UHN medial (UHN mean 11.43, MIMIC 11.83)."
        ),
    }
    ...
    for split in SPLITS:
        ...
        print(f"  E/e':     mean={s['mean']} std={s['std']} median={s['median']}  range=[{s['min']}, {s['max']}]")
    ...
    task_meta_path = OUT_SPLIT_DIR / "task_meta.json"
    with task_meta_path.open("w") as fh:
        json.dump(task_meta, fh, indent=2)
    print(f"Wrote task_meta: {task_meta_path}")
