...
- **`claude/chicago/`** — UCMC (University of Chicago) external validation inference guides for Teodora (Dora). 8 docs covering 94 runs (4 models × 24 tasks, minus 4 where only G/L-K have probes): `valve_severity.md` (4 classification tasks, view-filtered), `lv_function.md` (LVEF/EDV/ESV/CO regression), `hemodynamic.md` (AOV Mean Grad/Vmax cross-modal regression), `rv_function.md` (TAPSE/RVSP/RV FAC/RV S' regression), `diastolic.md` (MV e' medial regression), `disease_detection.md` (7 binary disease tasks, view-filtered), `diastolic_function.md` (4-class diastolic grading, view-filtered), `trajectory.md` (5 trajectory prediction tasks: LVEF trajectory 3-class, LVEF/MR/TAPSE/TR onset binary, view-filtered). All use frozen d=1 attentive probes, 4 models (EchoJEPA-G, L-K, EchoPrime, PanEcho; TAPSE/TR onset have G and L-K only). Probes on GDrive: `echo_foundation/nature_medicine/chicago/probes/{valve_severity,lv_function,hemodynamic,rv_function,diastolic,disease_detection,diastolic_function,trajectory}/`. Role doc at `uhn_echo/nature_medicine/context_files/roles/teodora.md`
