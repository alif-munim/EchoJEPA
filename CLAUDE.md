...
- **`claude/rebuttals/`** — ICML rebuttal (scores 2/3/3/4). See `rebuttals/README.md` for the full file index. Key docs: **`08-rebuttal-v2.md`** (strategy & framing), **`11-rebuttal-task-tracker.md`** (canonical task list — start here for what to do next), `10-rebuttal-experiment-results.md` (consolidated results & key findings), `09-three-way-comparison-results.md` (JEPA/BYOL/MAE 50ep detail), `12-checkpoint-reference.md` (all checkpoint paths). Reviewer concerns: novelty, segmentation, speckle validation, contrastive comparison. Key updates: UHN approved EchoJEPA-G gated release; BYOL-Video controlled baseline (3-way comparison); CKA speckle invariance and frame shuffling are the Tier 1 experiments
