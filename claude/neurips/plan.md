...
- [x] **SALT 6-condition frame shuffling (2026-04-08)** — SALT-S2-e79 through all 6 conditions. **Cliff profile:** clean 0.293, tubelet/matched ≈ 0.29 (flat under local disruption), reverse 0.206 (−30%), shuffle/matched_frame collapse to −0.41/−0.44 (−250% drop, largest of any model). Four qualitatively distinct profiles now populate §4.1 (JEPA gentle / BYOL steep / MAE flat / SALT cliff). SALT is the only method to go negative, and its clean R² is already below MAE's. Writeup: `experiments/6-condition-shuffling.md` § SALT-S2-e79 Results; cross-model table: `paper-outline.md` §4.1.
