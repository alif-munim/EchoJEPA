...
1. **V-Pixel and MAE are structurally different pixel-reconstruction patterns.** Probing the V-Pixel teacher directly at e24/e49/e74/e99 shows clean R² reaching 0.657 at e99 (tied with JEPA at e100, 0.650, and well above MAE at e99, 0.467), but with matched-frame ΔR² = −1.143 and Δr = −0.265 — the opposite pattern from MAE (ΔR² = −0.027, Δr = −0.029 at e99). V-Pixel and MAE share the pixel-reconstruction objective but differ in masking protocol (SALT's multi-block 0.15+0.7 spatial + 1.0 temporal vs MAE's 90% tube mask) and per-patch normalization; this design difference places them in different attentive-probe patterns at every matched checkpoint. V-Pixel at e99 is closer to BYOL's fragile utilization in direction but 4× larger in magnitude (BYOL e100 ΔR² = −0.279, Δr = −0.059). Extending the V-Pixel teacher therefore does not produce a MAE-like "encoded but disused" teacher whose pattern SALT could inherit — it produces a fragile-utilization teacher.

2. **SALT students distill a fragile teacher into a more-fragile student.** At the prescribed canonical configuration (e_salt = 99, V-Pixel e20 teacher), the teacher is too under-trained to probe meaningfully and the student sits at R² = 0.414 with Δr = −0.024 (calibration-failure-dominated collapse). As the teacher trains longer (extended-teacher trajectory e_salt = 103 → 179), the student's clean R² drifts non-monotonically and the matched-frame ΔR² deepens monotonically from −0.024 to −0.927, with Δr moving from −0.008 to −0.236 (ranking-meaningful). The student therefore does not recover MAE's encoded-but-disused pattern — it preserves the teacher's fragility with additional clean-R² loss from the distillation step (teacher R² 0.657 → student R² 0.445, drop of 0.21).

3. **At matched e_S1, target type dominates teacher quality.** The V-Pixel and JEPA teachers reach nearly identical standalone clean R² at e100 (0.657 and 0.650) but opposite matched-frame profiles (Δr = −0.265 vs −0.028). Their SALT S2 students differ by 2× in distillation cost:
