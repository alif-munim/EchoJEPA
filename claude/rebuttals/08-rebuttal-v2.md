...
| 5 | **Cross-view representation similarity (RVSP multi-view)** | ALL (world model) | ~30 min | High (tests view-invariance) |
...
**Cross-modality generalization:** Both the evaluation protocol and perturbation framework generalize beyond echocardiography. The physics-based perturbations (depth attenuation, acoustic shadow, haze) model phenomena common to ALL ultrasound imaging — obstetric, musculoskeletal, abdominal, vascular. The temporal perturbations apply to any video-based ultrasound analysis. The noise-filtering hypothesis predicts latent prediction should outperform pixel reconstruction in any speckle-dominated modality, making EchoBench a template for evaluating foundation models across the full spectrum of ultrasound applications. This broadens the impact from "echocardiography benchmark" to "ultrasound evaluation framework."
