...
Usage:
    TMPDIR=/tmp LD_LIBRARY_PATH=/opt/conda/lib:$LD_LIBRARY_PATH python scripts/neurips/noised_segmentation.py \
        --encoder_type vjepa \
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/neurips/noised_segmentation.py \
        --encoder_type vjepa --encoder_checkpoint checkpoints/echojepa-l-pt50.pt \
        --encoder_model_name vit_large \
        --decoder_checkpoint results/segmentation/echojepa_l_pt50/lr5e-02_wd1e-04/best_decoder.pt \
        --device cuda:0 --label echojepa_l_pt50 &

    CUDA_VISIBLE_DEVICES=1 python scripts/neurips/noised_segmentation.py \
    ...
    CUDA_VISIBLE_DEVICES=2 python scripts/neurips/noised_segmentation.py \
...
def main():
    ...
    if args.output is None:
        args.output = f"scripts/neurips/samples/{args.label}_noised_segmentation.csv"
