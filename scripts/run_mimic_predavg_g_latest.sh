#!/bin/bash
# Re-run pred avg for all EchoJEPA-G MIMIC outcome tasks using latest.pt.
# Runs 2 tasks in parallel on split GPUs (0-3 and 4-7).
# Saves per-clip predictions + attentive pooler outputs (clip_outputs.npz).
#
# Usage:
#   nohup bash scripts/run_mimic_predavg_g_latest.sh > logs/mimic_predavg_g_latest.log 2>&1 &
...
# 10 tasks (in_hospital_mortality excluded — no checkpoint)
TASKS=(
    mortality_1yr
    mortality_90d
    mortality_30d
...
run_one() {
    ...
    DEVICES="$devices" MASTER_PORT="$port" \
        bash scripts/run_mimic_pred_avg.sh --models "$MODEL" "$task" 2>&1 || {
...
# =============================================================================
log "=========================================="
log "MIMIC Pred Avg (G, latest.pt) — START"
log "Tasks: ${TASKS[*]}"
log "=========================================="
...
log ""
log "=========================================="
log "MIMIC Pred Avg (G, latest.pt) — ALL DONE"
log "=========================================="
